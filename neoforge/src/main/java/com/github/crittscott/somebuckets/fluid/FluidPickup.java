 ...
 * keeps its fluid.
 *
 * <p>NeoForge's {@link BucketPickupHandlerWrapper} presents the vanilla {@link BucketPickup}
 * contract as an {@link IFluidHandler}, so {@link #available} reports what one unit of pickup would
 * yield without changing anything. A caller decides whether that content is acceptable, checks
 * protection, and only then calls {@link #take}. NeoForge dropped Forge's legacy {@code IFluidBlock}
 * / {@code FluidBlockWrapper}; modern modded source blocks implement {@link BucketPickup} directly.
 *
...
public final class FluidPickup {
     ...
     * when asked what it holds, but only a source is actually collectable. The wrapper forwards
     * {@code player} to {@link BucketPickup#pickupBlock}; it may be null for a simulate-only query.
     */
    @Nullable
    private static IFluidHandler handlerFor(Level level, BlockPos pos, BlockState state,
        ...
        if (block instanceof BucketPickup pickup && state.getFluidState().isSource()) {
            return new BucketPickupHandlerWrapper(player, pickup, level, pos);
        }
        return null;
    }
    ...
    public static FluidStack available(Level level, BlockPos pos) {
        IFluidHandler handler = handlerFor(level, pos, level.getBlockState(pos), null);
    ...
    public static FluidStack take(Level level, BlockPos pos, FluidStack expected, @Nullable Player player) {
        ...
        IFluidHandler handler = handlerFor(level, pos, state, player);
        if (handler == null) return FluidStack.EMPTY;
