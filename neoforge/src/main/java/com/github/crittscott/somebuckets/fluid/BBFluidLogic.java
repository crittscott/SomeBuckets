...
public class BBFluidLogic {
    ...
    public boolean tryPlacePowder(Level level, BlockHitResult hit, ItemStack stack,
        ...
        // Player use path: CommonHooks.onPlaceItemIntoWorld has armed snapshot capture and defers
        // EntityPlaceEvent until after this method returns. Its held-stack rollback restores from a
        // live DataComponentMap reference and so cannot undo our custom-data debit, which would leave
        // the bucket drained after a canceled placement. Fire the place event here, observe the
        // cancellation before debiting, and finalize the captured snapshot so the outer wrapper does
        // not process it a second time.
        if (!((BlockItem) Items.POWDER_SNOW_BUCKET).place(placement).consumesAction()) return false;
