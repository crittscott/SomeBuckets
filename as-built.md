...
The Junk Bucket keeps the vanilla generated item geometry and is wrapped after baking, in `ModelEvent.ModifyBakingResult`, with an override handler that draws its contents. `client/JunkIconLayout` borrows each stored stack's own atlas sprite — the front-face quad of its baked model, falling back to the model's particle icon — and lays it out as a rotated square clipped to the bucket's mouth, emitting the clipped polygon as triangles in `BakedQuad` form. The mouth is not a constant: `client/BucketMouth` reads `junk_bucket_opening.png` — a mask of its own, separate from the fluid buckets' content mask — and reduces it to horizontal spans in item-model space, so the region that accepts contents is exactly the shape the art describes. Only each row's outermost opaque pixels are read, so a row is filled between them. Placement comes from a generator seeded on the stored item's registry name and its index, so an arrangement is scattered but stable across frames and across buckets holding the same things; it does not avoid collisions, and contents overlap as they fill up. Height is not random: each icon hangs from the rim by its own drawn pixels. `client/SpriteBounds` measures the box a sprite actually draws within, since most icons leave a transparent margin and several fill only a diagonal band, and the tallest corner of that box once rotated is what the icon is positioned to clear. Only the sides and bottom of an icon are ever cut, which read as the contents passing behind the bucket; a cut across the top read as damage to the item. The oldest stack is drawn nearest the viewer and last, matching FIFO ejection order. Item rendering discards the color baked into a quad's vertices, so each icon carries a tint index of its own instead, numbered past anything a generated item model produces; the Junk Bucket's item color handler resolves those indices back to the stored items' own colors and leaves the vessel's layers alone. An item whose model reports a custom renderer, or whose sprite resolves to the missing texture, is drawn as cobblestone. Resolved models are cached under the stack's tag, which is cheaper to compare than the deserialized stacks, bounded, and discarded with the baked model on resource reload.
...
- `JunkIconLayout` emits its quads in the vertex order `Direction.SOUTH` uses — top-left, bottom-left, bottom-right, top-right — and relies on stored item icons living on the block atlas alongside the bucket's own layers. It also asks the item renderer for other items' models while resolving, so its cache is populated outside the map lookup rather than through `computeIfAbsent`.
- A `BakedModelWrapper` that changes an item's quads has to defeat two inherited behaviors that would otherwise discard it silently. `applyTransform` must return `this`: the inherited version returns whatever the delegate returns, which is the delegate, and `ItemRenderer` uses that return value for the render passes and quads that follow — so the wrapper is dropped one call before it would have been used. The three-argument `getQuads` must also be overridden, since item rendering calls that form rather than Forge's overload taking model data and a render type. `JunkContentsModel` and `NbtFluidContainerModel` do both, and key their quad caches by which `getQuads` overload was called, since a delegate may answer them differently.
